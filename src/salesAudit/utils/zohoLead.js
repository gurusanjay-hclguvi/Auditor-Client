...
export function fromZohoLead(raw) {
  ...
  return {
    ...
    zenId: text(raw.zenId ?? raw.zenID),
    onboardCoordinator: text(raw.onboardCoordinator),
    paysInSameMonth: text(raw.willLeadPayinSameMonth),
    rechecks: toRechecks(raw),
    // Filterable Zoho fields (see utils/leadFilters.js)
    superleapId: text(raw.superleapId),
    product: text(raw.product),
    enrolledAt: parseZohoDate(raw.dateOfEnrollment),
    admissionForm: text(raw.admissionDetails?.admissionform),
    batchName: text(raw.batchData?.batchName),
    batchStartAt: parseZohoDate(raw.batchData?.startDate),
    batchStartDate: displayZohoDate(raw.batchData?.startDate),
    sourceMedium: text(raw.medium),
    sourceCampaign: text(raw.campaign),
    salesFrom: text(raw.salesFrom),
    termsAndConditions: text(raw['terms&conditions']),
    promoCode: text(raw.promoCode),
    paymentMode: text(payments.find((payment) => payment.modeOfPayment)?.modeOfPayment),
  }
}
...
function fromApiLead(raw) {
  ...
  return {
    ...
    enrolledAt: raw.sapEnteredAt ?? null,
    product: text(raw.course),
    superleapId: '',
    admissionForm: '',
    batchName: '',
    batchStartAt: null,
